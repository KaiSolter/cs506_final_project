...
def get_win_rate(username):
    ...
    for line in response.iter_lines():
        if line:
            ...
            # Only consider games where the user is the white player
    ...
    if game_count == 0:
        print(f"No blitz games found for user: {username} as white.")
    ...
    print(f"Win Rate for {username} as White in Blitz: {win_rate:.2f}%")
    print(f"Lose Rate for {username} as White in Blitz: {lose_rate:.2f}%") 
    print(f"Draw Rate for {username} as White in Blitz: {draw_rate:.2f}%")
    print(f"Wins: {win_count}, Draws: {draw_count}, Losses: {loss_count}, Total Blitz Games as White: {game_count}")
